...
namespace SpMP
{
  ...
  /***
   * 这个函数将矩阵划分为L和U，也就是将矩阵的上三角和下三角分开存储。
   */
  void splitLU(CSR &A, CSR *L, CSR *U)
  {
    ...
    L->diag = MALLOC(double, L->m);
    U->diag = MALLOC(double, U->m);
...
#pragma omp single
    ...
    // for(int i=0; i < A.m; i++)
    // {
    //   printf("L rowptr %d: %d, A rowptr %d\n", i, L->rowptr[i], A.rowptr[i]);
    // }
...
#pragma omp barrier
        ...
        for (int i = iBegin; i < iEnd; ++i)
        {
          ...
          if (A->extptr)
            (*symExtPtr)[i] = (*symRowPtr)[i] + cnts[i];
